...
export class AudioEngine {
  ...
  start() {
    ...
    this._loadSfx();                   // pull in the real foley samples (async)
  }

  // ── ElevenLabs foley samples ── real recorded-quality one-shots replace the
  // synthesized foley. Loaded async after the first tap; until ready (or if a
  // sample is missing) the procedural fallback in each method still plays, so
  // nothing ever goes silent. Generated by tools/gen_sfx.mjs.
  _loadSfx() {
    const C = this.ctx;
    const names = [
    ...
    names.forEach(async (name) => {
      try {
        const r = await fetch('sfx/' + name + '.mp3');
        if (!r.ok) return;
  ...
  // play a foley sample positioned in the world (HRTF panner toward its bearing).
  _playAt(base, x, z, { gain = 1, rate = 1, n = 1, ref = 12, max = 150, rolloff = 0.9 } = {}) {
    const C = this.ctx; if (!C || x === undefined) return false;
    const name = n > 1 ? `${base}_${1 + Math.floor(Math.random() * n)}` : base;
    const buf = this._sfx[name] || this._sfx[base];
    if (!buf) return false;
    const src = C.createBufferSource(); src.buffer = buf;
    src.playbackRate.value = rate * (0.95 + Math.random() * 0.1);
    const pan = this._panner(x, z); pan.refDistance = ref; pan.maxDistance = max; pan.rolloffFactor = rolloff;
    const g = C.createGain(); g.gain.value = gain;
    src.connect(g).connect(pan); pan.connect(this.spatialBus);
    try { src.start(); } catch (e) { return false; }
    return true;
  }
  ...
  twang(power = 1) {
    if (!this.started) return;
    if (this._play('bow_release', { gain: 0.55 + 0.45 * Math.min(1, power), rate: 0.96 + 0.12 * Math.min(1, power), n: 2 })) return;
    const C = this.ctx, t = C.currentTime;
  ...
  snapAt(x, z, player, vol = 0.8) {
    if (!this.started || this.muted) return;
    if (this._playAt('twig_snap', x, z, { gain: vol, n: 2, ref: 8, max: 90, rolloff: 1.1 })) return;
    const C = this.ctx, t = C.currentTime;
  ...
  bearScream(x, z, vol = 1.2) {
    if (!this.started || this.muted) return;
    if (this._playAt('bear_growl', x, z, { gain: vol, n: 2, ref: 14, max: 170, rolloff: 0.7 })) return;
    const C = this.ctx, t = C.currentTime;
  ...
  bearCharge(x, z, vol = 1) {
    if (!this.started || this.muted) return;
    if (this._playAt('bear_charge', x, z, { gain: vol, ref: 12, max: 150, rolloff: 0.8 })) return;
  ...
  bearRustle(x, z, vol = 0.6) {
    if (!this.started || this.muted) return;
    if (this._playAt('bear_rustle', x, z, { gain: vol, ref: 10, max: 110, rolloff: 1.0 })) return;
    const C = this.ctx, t = C.currentTime;
  ...
  wakeBreath() {
    if (!this.started || this.muted) return;
    if (this._play('breath', { gain: 0.72, rate: 0.82, n: 2 })) return;   // deeper, slower = the waking gasp
    const C = this.ctx, t = C.currentTime;
  ...
  cue(step = 0) {
    // DISABLED — this was the 8-bit "boop" on wake/control-reveal/mushroom.
    // The control reveal is visual; no chiptune beep. (Kept as a no-op so every
    // existing cue() call site stays harmless.)
    return;
  }
  ...
  breathAt(x, z, vol = 0.5) {
    if (!this.started || this.muted) return;
    if (this._playAt('breath', x, z, { gain: vol, rate: 0.7, n: 2, ref: 10, max: 110 })) return;   // slowed = a big animal's huff
    const C = this.ctx, t = C.currentTime;
    const pan = this._panner(x, z); pan.connect(this.spatialBus);
    const src = C.createBufferSource(); src.buffer = this._shotNoise; src.loop = true;
  ...
  thud() {                          // player hurt — gut punch
    if (!this.started) return;
    if (this._play('thud', { gain: 0.95 })) return;
    const C = this.ctx, t = C.currentTime;
